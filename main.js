...
new ScrollMagic.Scene({
  ...
  triggerHook: 0.91, //뷰포트의 세로축 0~1 중 .8지점에 보이면
})
...
function habitIf(index, tf) {
  if (index === 0) {
    if (tf) habitH3P(1, "운동", habitP1);
    else habitH3P(0, "운동", habitP1);
  } else if (index === 1) {
    if (tf) habitH3P(1, "영양제 섭취", habitP2);
    else habitH3P(0, "영양제 섭취", habitP2);
  } else if (index === 2) {
    if (tf) habitH3P(1, "공부", habitP3);
    else habitH3P(0, "공부", habitP3);
  } else if (index === 3) {
    if (tf) habitH3P(1, "기상", habitP4);
    else habitH3P(0, "기상", habitP4);
  } else if (index === 4) {
    if (tf) habitH3P(1, "독서 및 명상", habitP5);
...
교훈을 남겨서, 하루에 공부시간을 1분씩 늘리는 계획을 세우고 '어제 한 것보다 딱 1분만 더하면 되잖아'라는 생각으로 공부를 했습니다. 공부시간이 1시간 밖에 안될 땐 가족들에게 무시를
...
마지막으로 기상시간을 정했으면 무조건 일단 일어나고 졸리다면 재밌는 것을 해서 낮잠을 참고 밤에 자보세요. 그럼 기상 습관을 만드는 데 도움이 될 것입니다.
`;
...
당신의 삶에 축복이 있길 진심으로 바라고, 세상이 사랑과 감사만으로 넘치는 상상만 해도 좋은 그런 세상을 만들기 위해 노력하는 사람이 많아지길 기원합니다.
